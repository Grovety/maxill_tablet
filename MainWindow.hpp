...
public:
	...
	Q_INVOKABLE void sendRequestSetupDataLogger (
		const QString &label, const int sampleRate, const double startTemp, const double startPress,
		const int confirmationPeriod, const quint64 timestamp,
		const double targetTemp, const double targetPress, const double criticalTemp,
		const double criticalPress, const int targetTime, const int deivationPeriod);
private:
